...
MeshCore has four firmware types that are not available on other LoRa systems.
...
BLE Companion firmware runs on a supported LoRa device and connects to a smart device running the Android or iOS MeshCore client over BLE: https://meshcore.co.uk/apps.html
...
- A T-Deck running unlocked/registered MeshCore firmware. Remote server administration is enabled through registering your T-Deck with Ripple Radios. It is one of the ways to support MeshCore development. You can register your T-Deck at: https://buymeacoffee.com/ripplebiz/e/249834
...
**A:** T-Deck uses the same key the smartphone apps use but in base64: `izOH6cXN6mrJ5e26oRXNcg==` (The third character is the capital letter 'O', not zero 0)
...
There is also a modified script that adds additional error handling and parallel downloads available on Discord.
...
**A:** The smartphone app key is in hex: `8b3387e9c5cdea6ac9e5edbaa115cd72`

T-Deck uses the same key but in base64: `izOH6cXN6mrJ5e26oRXNcg==` (The third character is the capital letter 'O', not zero 0)
...
https://www.youtube.com/watch?v=WJvg6dt13hk
...
#### For ESP-based devices (e.g. Heltec V3):

1. Download firmware file from flasher.meshcore.co.uk
...
2. Run:
   ```bash
   wget https://flasher.meshcore.dev/releases/download/companion-v1.7.1/Heltec_v3_companion_radio_ble-v1.7.1-165fb33.bin
   ```

   If the above wget command only downloads a very small file (10K bytes instead of more than 100K bytes), use this command instead:
   ```bash
...
3. Confirm the ttyXXXX device path on your Raspberry Pi:
   - Go to `/dev` directory, run `ls` command to find confirm your device path
   - They are usually `/dev/ttyUSB0` for ESP devices

4. For ESP-based devices, install esptool from the shell:
   ```bash
   pip install esptool --break-system-packages
   ```

5. To flash, use the following command:
   - For non-merged bin:
     ```bash
...
#### For nRF devices (e.g. RAK, Heltec T114):

1. Download firmware file from flasher.meshcore.co.uk
   - You need the ZIP version for the adafruit flash tool
   - Example: `RAK_4631_companion_radio_ble-v1.7.1-165fb33.zip`

2. Run:
   ```bash
   wget https://flasher.meshcore.dev/releases/download/companion-v1.7.1/RAK_4631_companion_radio_ble-v1.7.1-165fb33.zip
   ```

3. Confirm the ttyXXXX device path on your Raspberry Pi:
   - They are usually `/dev/ttyACM0` for nRF devices

4. For nRF-based devices, install adafruit-nrfutil:
   ```bash
   pip install adafruit-nrfutil --break-system-packages
   ```

5. Use this command to flash the nRF device:
   ```bash
...
#### Managing via USB Serial

To manage a repeater or room server connected to a Pi over USB serial using shell commands, you need to install picocom:

```bash
sudo apt install picocom
```

To start managing your USB serial-connected device using picocom:

```bash
picocom -b 115200 /dev/ttyUSB0 --imap lfcrlf
```

Reference repeater and room server command line commands on MeshCore github wiki: https://github.com/meshcore-dev/MeshCore/wiki/Repeater-&-Room-Server-CLI-Reference
...
A Python script to send meshcore debug and packet capture data to MQTT for analysis: https://github.com/Andrew-a-g/meshcoretomqtt

#### 5.14.2. MeshCore for Home Assistant

A custom Home Assistant integration for MeshCore mesh radio nodes. It allows you to monitor and control MeshCore nodes via USB, BLE, or TCP connections: https://github.com/awolden/meshcore-ha

#### 5.14.3. Python MeshCore

Bindings to access your MeshCore companion radio nodes in python: https://github.com/fdlamotte/meshcore_py

#### 5.14.4. meshcore-cli

CLI interface to MeshCore companion radio over BLE, TCP, or serial. Uses Python MeshCore above: https://github.com/fdlamotte/meshcore-cli

#### 5.14.5. meshcore.js

A JavaScript library for interacting with a MeshCore device running the companion radio firmware: https://github.com/liamcottle/meshcore.js
...
1. Connect USB-C cable to your device, per your device's instruction, get it to flash mode:
   ...
   - For Xiao nRF52, click the reset button once. If that doesn't work, quickly double click the reset button twice. If that doesn't work, disconnect the board from your PC and reconnect again (seeed studio wiki)

2. A new folder will appear on your computer's desktop

3. Download the `flash_erase*.uf2` file for your device on flasher.meshcore.co.uk:
...
6. Go to https://flasher.meshcore.co.uk/, click Console and select the serial port for your connected device
...
**Note:** Starting in firmware version 1.7.0, there is a CLI Rescue mode. If your device has a user button (e.g. some RAK, T114), you can activate the rescue mode by hold down the user button of the device within 8 seconds of boot. Then you can use the 'Console' on flasher.meshcore.co.uk

### 6.7. Q: WebFlasher fails on Linux with failed to open

**A:** If the usb port doesn't have the right ownership for this task, the process fails with the following error: `NetworkError: Failed to execute 'open' on 'SerialPort': Failed to open serial port.`
...
4. Go to the Command Line tab, type `start ota` and hit enter. You should see `OK` to confirm the repeater device is now in OTA mode

5. Run the DFU app, tab Settings on the top right corner

6. Enable `Packets receipt notifications`, and change `Number of Packets` to 10 for RAK, 8 for T114. 8 also works for RAK.

7. Select the firmware zip file you downloaded

8. Select the device you want to update. If the device you want to update is not on the list, try enabling OTA on the device again. If the device is not found, enable `Force Scanning` in the DFU app

9. Tab the Upload to begin OTA update

10. If it fails, try turning off and on Bluetooth on your phone. If that doesn't work, try rebooting your phone.

11. Wait for the update to complete. It can take a few minutes.
...
3. Go to the Command Line tab, type `start ota` and hit enter. You should see `OK` to confirm the repeater device is now in OTA mode

4. The command `start ota` on an ESP32-based device starts a wifi hotspot named `MeshCore OTA`

5. From your phone or computer connect to the 'MeshCore OTA' hotspot

6. From a browser, go to http://192.168.4.1/update and upload the non-merged bin from the flasher
